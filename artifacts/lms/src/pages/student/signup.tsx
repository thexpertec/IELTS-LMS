...
export default function StudentSignup() {
  ...
  const { data: tenantInfo } = useQuery({
    ...
    queryFn: async () => {
      const r = await fetch("/api/tenant/public", { credentials: "include" });
      if (!r.ok) return null;
      return r.json() as Promise<{ id: number; name: string } | null>;
    },
    staleTime: Infinity,
  });
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      const r = await fetch("/api/auth/student-register", {
        ...
        credentials: "include",
        body: JSON.stringify({ name: name.trim(), email: email.trim(), password, tenantId: tenantInfo?.id }),
      });
      const data = await r.json() as { message?: string };
      if (!r.ok) { setError(data.message ?? "Registration failed"); return; }
      setLocation("/student/dashboard");
    } catch {
    ...
    <div className="min-h-screen flex flex-col bg-[#0d1b60]">
      ...
      <div className="bg-[#CC0000] shrink-0">
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-9 flex items-center justify-end gap-0">
          <a href="/lms/" className="px-4 h-9 flex items-center text-white text-sm font-medium hover:bg-white/15 transition-colors">Admin Login</a>
          <a href="/lms/student/login" className="px-4 h-9 flex items-center text-white text-sm font-medium hover:bg-white/15 transition-colors">Student Login</a>
        </div>
      </div>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col lg:flex-row w-full max-w-7xl mx-auto px-4 md:px-8 py-12 gap-8 items-start lg:items-center">
          ...
          <div className="bg-white w-full lg:w-[420px] shrink-0 p-8 shadow-xl">
            <h1 className="text-2xl font-bold text-[#0d1b60] mb-1">Create Account</h1>
            <p className="text-gray-500 text-sm mb-7">
              Join {tenantInfo?.name ?? "IELTS Academy LMS"} and start learning today.
            </p>
            ...
            <div className="mt-6 pt-5 border-t border-gray-100">
              <p className="text-sm text-gray-500 text-center">
                ...
                <a href="/lms/student/login" className="font-semibold text-[#0d1b60] hover:text-[#CC0000] transition-colors">
                  Sign in
                </a>
              </p>
            </div>
