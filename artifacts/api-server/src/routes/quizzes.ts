...
router.put("/quizzes/:id", async (req, res): Promise<void> => {
  ...
  const [quiz] = await db
    ...
    .set({
      ...
      ...("videoUrl" in req.body && { videoUrl: typeof req.body.videoUrl === "string" ? req.body.videoUrl || null : null }),
      ...("imageUrls" in req.body && { imageUrls: Array.isArray(req.body.imageUrls) ? req.body.imageUrls : null }),
      ...("audioUrls" in req.body && { audioUrls: Array.isArray(req.body.audioUrls) ? req.body.audioUrls : null }),
